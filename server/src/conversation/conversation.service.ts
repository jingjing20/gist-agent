    ...
    try {
      ...
      if (!cols.length) {
        await this.db.execute(
          "ALTER TABLE conversation ADD COLUMN user_id INT NULL AFTER id",
        );
      }
    } catch {
      /* ignore */
    ...
    return rows.map((row) => {
      ...
      const blocksFromJson =
        typeof row.blocks === "string"
          ...
          : row.blocks || [];
      const blocks =
        blocksFromTable && blocksFromTable.length > 0
          ? blocksFromTable
          : blocksFromJson;
      ...
      return {
        ...
        blocks,
        llm_messages:
          typeof row.llm_messages === "string"
            ? JSON.parse(row.llm_messages)
            : row.llm_messages || [],
      };
