...
- SQL 报错或结果为空时，如实说明现象并给出可能原因或下一步建议，绝不编造数据填补。`;
  ...
  async buildMessages(
    ...
    const budgetForHistory =
      MAX_CONTEXT_TOKENS - systemTokens - RESPONSE_RESERVE - TOOLS_RESERVE;
  ...
  private toHistoryEntry(msg: Message): HistoryEntry {
    ...
    const hasLlmMessages =
      Array.isArray(msg.llm_messages) && msg.llm_messages.length > 0;

    const fullMessages = hasLlmMessages
      ? (msg.llm_messages as ChatMessage[]).map((m) => this.dehydrateMessage(m))
      : [compactMsg];
    const fullTokens = hasLlmMessages
  ...
  private dehydrateMessage(msg: ChatMessage): ChatMessage {
    ...
    if (msg.role === "assistant" && (msg as any).tool_calls) {
      const calls = (msg as any).tool_calls.map((c: any) =>
        this.dehydrateToolCall(c),
      );
