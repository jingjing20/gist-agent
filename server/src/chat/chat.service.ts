...
export class ChatService {
  ...
  async handleChat(
    ...
    if (
      datasourceId &&
      !(await this.datasourceService.canAccess(datasourceId, userId))
    ) {
    ...
    try {
      await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error("分析超时，请尝试简化问题后重试")),
            CHAT_TIMEOUT_MS,
          ),
        ),
      ]);
  ...
  private async runAgentLoop(
    ...
    for (let iteration = 0; iteration < MAX_AGENT_ITERATIONS; iteration++) {
      ...
      for (const tc of validToolCalls) {
        ...
        const title =
          name === "execute_sql_query" ? "[SQL 生成]" : `[工具调用] ${name}`;
        const lastLog = [...blocks]
          .reverse()
          .find((b) => b.type === "log" && b.title === title);
        if (lastLog) {
          lastLog.content =
            name === "execute_sql_query"
