...
export class SqlQueryTool implements Tool {
  ...
  async execute(
    ...
    try {
      const r = await this.sqlExecutor.execute(
        sql,
        ctx.datasourceId,
        ctx.userId,
      );
      ...
      if (serialized.length > MAX_TOOL_RESULT_CHARS && resultRows.length > 1) {
        ...
        const fitCount = Math.max(
          1,
          Math.floor(MAX_TOOL_RESULT_CHARS / avgSize),
        );
      ...
      const toolResult = r.wasFixed
        ...
        : truncationNote
          ? JSON.stringify({
              systemMessage: truncationNote.trim(),
              data: resultRows,
            })
