...
export class SemanticDistillerService {
  ...
  async updateStateAsync(
    ...
    this.distillAndUpdate(conversationId, userId, userMessage).catch((e) => {
      this.logger.warn(
        `Semantic Distillation failed for conv ${conversationId}: ${e.message}`,
      );
    });
  }

  private async distillAndUpdate(
    conversationId: string,
    userId: number,
    userMessage: string,
  ) {
    ...
    if (conv.semantic_state) {
      const raw =
        typeof conv.semantic_state === "string"
          ? JSON.parse(conv.semantic_state)
          : conv.semantic_state;
