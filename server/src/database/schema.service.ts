...
export class SchemaService {
  ...
  async getDatabaseSchemaPrompt(
    ...
    const uploadedTableNames = await this.getUserUploadedTableNames(
      datasourceId,
      userId,
    );
    ...
    const prompt = await this.fetchSchemaPromptFromLocal(
      allowedTables,
      uploadedTableNames,
    );
   ...
   */
  private async fetchSchemaPromptFromLocal(
    allowedTables: string[],
    uploadedTableNames: string[],
  ): Promise<string> {
    const dbNameRows = await this.db.query<any[]>(
  ...
  private async getUserUploadedTableNames(
    datasourceId: number | null,
    userId: number,
  ): Promise<string[]> {
    const actualDatasourceId =
      datasourceId ?? (await this.getLocalDatasourceId());
  ...
  private formatSchemaRows(rows: any[], uploadedTableNames: string[]): string {
    ...
    const tablesMap = new Map<
      string,
      { comment: string; columns: string[]; isUploaded: boolean }
    >();
