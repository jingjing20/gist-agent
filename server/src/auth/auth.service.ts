...
export class AuthService {
  ...
  async requestReset(email: string, origin?: string): Promise<void> {
    ...
    await this.db.execute(
      "INSERT INTO password_reset_token (user_id, token, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 15 MINUTE))",
      [userId, token],
    );
  ...
  async resetPassword(token: string, newPassword: string): Promise<void> {
    ...
    await this.db.withTransaction(async (conn) => {
      ...
      await conn.execute("UPDATE user SET password_hash = ? WHERE id = ?", [
        hash,
        userId,
      ]);
      await conn.execute(
