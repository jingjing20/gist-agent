...
export class UserService {
    ...
    async searchByEmail(q: string, limit = 20): Promise<{ id: number; email: string; name: string }[]> {
        ...
        const rows = await this.db.query<RowDataPacket[]>('SELECT id, email, name FROM user WHERE email LIKE ? ORDER BY email LIMIT ?', [`%${q.trim()}%`, limit]);
    ...
    async updateProfile(userId: number, name: string): Promise<void> {
        ...
        await this.db.execute('UPDATE user SET name = ? WHERE id = ?', [name.trim(), userId]);
    }

    async updatePassword(userId: number, newPassword: string): Promise<void> {
        if (!newPassword || newPassword.length < 6) throw new Error('密码至少 6 位');
        const hash = await bcrypt.hash(newPassword, SALT_ROUNDS);
        await this.db.execute('UPDATE user SET password_hash = ? WHERE id = ?', [hash, userId]);
    }
}
