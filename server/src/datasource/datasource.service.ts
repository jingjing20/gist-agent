...
export class DataSourceService {
    ...
    async findOne(id: number, userId: number): Promise<DataSource> {
        ...
        const rows = await this.db.query<RowDataPacket[]>('SELECT id, name, is_local, created_by, description, created_at FROM data_source WHERE id = ?', [id]);
    ...
    async create(
        ...
        const result = await this.db.execute(`INSERT INTO data_source (name, created_by, description) VALUES (?, ?, ?)`, [body.name, userId, body.description ?? null]);
        await this.db.execute('INSERT INTO datasource_permission (datasource_id, user_id, granted_by) VALUES (?, ?, ?)', [result.insertId, userId, null]);
    ...
    async update(
        ...
        if (fields.length > 0) {
            values.push(id);
            await this.db.execute(`UPDATE data_source SET ${fields.join(', ')} WHERE id = ?`, values);
        }
    ...
    async remove(id: number, userId: number): Promise<void> {
        const rows = await this.db.query<RowDataPacket[]>('SELECT is_local, created_by FROM data_source WHERE id = ?', [id]);
        if (!rows.length) throw new NotFoundException(`数据源 id=${id} 不存在`);
        const row = rows[0] as any;
        if (row.is_local) throw new BadRequestException('默认本地库不可删除');
        if (row.created_by !== userId) throw new ForbiddenException('仅创建人可删除');

        const tableRows = await this.db.query<RowDataPacket[]>('SELECT table_name FROM uploaded_table WHERE datasource_id = ?', [id]);
        ...
        try {
            try {
                ...
                for (let i = 0; i < rows.length; i += BATCH) {
                    ...
                    await this.db.execute(`INSERT INTO \`${tableName}\` (${columns.map((c) => `\`${c.name}\``).join(', ')}) VALUES ${placeholders}`, values);
                }
            }
            ...
            for (const col of columns) {
                ...
                await this.db.execute(`ALTER TABLE \`${tableName}\` MODIFY \`${col.name}\` ${col.type} COMMENT '${esc(comment)}'`);
            }

            const result = await this.db.execute('INSERT INTO uploaded_table (datasource_id, user_id, table_name, display_name) VALUES (?, ?, ?, ?)', [
                datasourceId,
                userId,
                tableName,
                displayName,
    ...
    async getUploadedTableNames(datasourceId: number, userId: number): Promise<string[]> {
        const rows = await this.db.query<RowDataPacket[]>('SELECT table_name FROM uploaded_table WHERE datasource_id = ?', [datasourceId]);
    ...
    async grant(datasourceId: number, targetUserId: number, grantorId: number): Promise<void> {
        ...
        await this.db.execute('INSERT IGNORE INTO datasource_permission (datasource_id, user_id, granted_by) VALUES (?, ?, ?)', [datasourceId, targetUserId, grantorId]);
    }
    ...
    async revoke(datasourceId: number, targetUserId: number, grantorId: number): Promise<void> {
        ...
        await this.db.execute('DELETE FROM datasource_permission WHERE datasource_id = ? AND user_id = ?', [datasourceId, targetUserId]);
    }
