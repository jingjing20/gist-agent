...
export class SchemaEnrichmentService {
  ...
  async enrichTableSchemaAsync(tableName: string): Promise<void> {
    this.enrichTableSchema(tableName).catch((err) => {
      this.logger.error(
        `Async enrichment failed for ${tableName}: ${err.message}`,
      );
    });
  }
...
${JSON.stringify(samples, null, 2)}`;
      ...
      this.logger.log(
        `Table [${tableName}] schema enrichment completed successfully.`,
      );
    } catch (error: any) {
      this.logger.warn(
        `Failed to enrich schema for table [${tableName}]: ${error.message}`,
      );
    }
  }
}
