...
function ensureUniqueColumnNames(rawNames: string[]): string[] {
  const sanitize = (s: string) =>
    s.replace(/[^a-zA-Z0-9_\u4e00-\u9fa5]/g, "_").replace(/^_+|_+$/g, "") ||
    "col";
...
export class DataSourceController {
  ...
  async uploadTable(
    ...
    if (!tableConfigs.length)
      throw new BadRequestException("请选择至少一个要上传的表");
    ...
    for (const config of tableConfigs) {
      ...
      if (ext === "csv") {
        ...
        if (rawRows.length > MAX_FILE_ROWS) {
          throw new BadRequestException(
            `Sheet「${sheetName}」行数超过上限 ${MAX_FILE_ROWS} 行`,
          );
        }
      }
      ...
      const result = await this.datasourceService.uploadTable(
        datasourceId,
        user.id,
        displayName,
        columns,
  ...
  async deleteTable(
    ...
    const dsId = await this.datasourceService.deleteUploadedTable(
      tableId,
      user.id,
    );
  ...
  async getSchema(
    ...
    return this.datasourceService["schemaService"].getStructuredSchema(
      id,
      user.id,
    );
  }
