<template>
  <div class="ds-view no-scrollbar">
    <div class="ds-header">
      ...
      <button class="add-ds-btn" @click="showCreateModal = true">
    ...
    <TableListModal 
      v-model="showingTablesDs"
      :tables="tables"
      :loading="tablesLoading"
      @delete-table="promptDeleteTable"
    />
...
async function loadPermissions() {
  ...
  try { grantedUsers.value = await dsStore.listPermissions(grantDs.value.id); } catch { grantedUsers.value = []; }
}
...
async function openTables(ds: DataSource) {
  ...
  if (ds.is_local) {
    ...
    try { tables.value = await dsStore.listTables(ds.id); } catch { tables.value = []; } finally { tablesLoading.value = false; }
  }
}

// --- Delete ---
const deleteConfirm = reactive({
  show: false, title: '', desc: '', type: '' as 'ds' | 'table',
  dsId: 0, tableId: 0,
});
