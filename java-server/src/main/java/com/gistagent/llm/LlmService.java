...
/**
 * Thin wrapper over the OpenAI Java SDK that matches the surface of the Nest LlmService: a blocking
 * {@link #chat} and a callback-style {@link #chatStream}. The underlying OpenAIClient is configured
 * in {@code OpenAIClientConfig}.
 *
 * <p>Note: LangSmith tracing is wired by wrapping the OpenAI client in the Nest version. Java has
 * no drop-in equivalent yet; if you need tracing, introduce the langsmith Java SDK in a later phase
 * and decorate the client bean there.
 */
@Service
public class LlmService {
  ...
  public String chatStream(
      String systemPrompt, String userPrompt, double temperature, Consumer<String> onChunk) {
    ChatCompletionCreateParams params =
        ChatCompletionCreateParams.builder()
            .model(model)
            .temperature(temperature)
