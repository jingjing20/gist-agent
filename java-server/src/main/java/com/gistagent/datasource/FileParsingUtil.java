 ...
 *
 * <p>Rules are kept byte-identical to avoid behavioral drift across servers: - strip zero-width /
 * NBSP characters, trim - turn thousands-separator strings ("1,234.56") into plain numerics - blank
 * strings become null so MySQL type inference is not poisoned - column names keep Chinese /
 * alphanumerics; everything else is _ ; duplicates get _1, _2 - type inference is conservative:
 * TEXT unless all non-null values are numeric
 */
public final class FileParsingUtil {
  ...
  private static final Pattern SAFE_COLUMN_NAME =
      Pattern.compile("^[a-zA-Z_\\u4e00-\\u9fff][a-zA-Z0-9_\\u4e00-\\u9fff]*$");
  ...
  /**
   * Conservative type inference: everything non-numeric -> TEXT. If every non-null sample parses as
   * a number, pick BIGINT for pure integers else DOUBLE. Pre-existing Nest behavior — do not widen
   * the decision tree.
   */
  ...
  public static List<String> ensureUniqueColumnNames(List<String> rawNames) {
    ...
    for (String raw : rawNames) {
      String sanitized =
          EDGE_UNDERSCORE
              .matcher(UNSAFE_CHAR.matcher(raw == null ? "" : raw).replaceAll("_"))
              .replaceAll("");
