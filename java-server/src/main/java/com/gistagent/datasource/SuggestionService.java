...
/**
 * Port of the Nest SuggestionService. LLM output is cached in
 * {@code datasource_suggestions} keyed by (datasource_id, user_id).
 *
 * Async methods delegate to a self-inject so Spring's proxy kicks in; we do not
 * swallow LLM errors silently in sync calls — the controller's getSuggestions
 * surface still returns 500 when generation actually fails on a cache miss.
 */
@Service
public class SuggestionService {
	...
	public SuggestionService(JdbcTemplate jdbc, SchemaService schema, LlmService llm, ObjectMapper mapper) {
		this.jdbc = jdbc;
		this.schema = schema;
		this.llm = llm;
		this.mapper = mapper;
	...
	public void triggerAsync(long datasourceId, long userId) {
		try {
			...
			log.warn("background suggestion generation failed for ds={} user={}: {}",
					datasourceId, userId, e.getMessage());
		}
	}
	...
	public void invalidateAndRegenerate(long datasourceId, long userId) {
		try {
			...
			log.warn("background suggestion invalidate+regen failed for ds={} user={}: {}",
					datasourceId, userId, e.getMessage());
		}
	}
	...
	private List<String> getFromCache(long datasourceId, long userId) {
		return jdbc.query(
				...
				rs -> {
					...
					try {
						return mapper.readValue(q, mapper.getTypeFactory()
	...
	private List<String> callLlm(String schemaPrompt) {
		...
		try {
			List<String> parsed = mapper.readValue(m.group(),
