...
/**
 * SMTP mail service. Lazy-init semantics match the Nest version: if SMTP is
 * misconfigured, the service starts fine and only throws on actual send.
 *
 * JavaMailSender is injected optionally because Spring's auto-config may skip
 * the bean when spring.mail.host is empty.
 */
@Service
	...
	public void sendResetEmail(String to, String resetUrl) {
		if (!isConfigured()) {
			throw ApiException.internal(
					"SMTP 未配置，无法发送邮件。请配置 SMTP_HOST / SMTP_USER / SMTP_PASS");
		}
