...
/** generate_chart：在 analyze_result 判断需要图表后调用。 把结构化图表数据以 {@code type=chart} 的 SSE 事件下发给前端。 */
@Component
public class GenerateChartTool implements Tool {
                  ...
                  Map.of(
                      ...
                      "xAxis",
                          Map.of(
                              ...
                              "description",
                                  "各数据点的标签；bar/line/scatter 为 X 轴分类，pie/funnel 为各扇区名称，radar 为多维指标名称（必填，长度需对应数据）"),
                      "series",
                          Map.of(
                              "type", "array",
                              "description", "数据系列",
                              "items",
                                  Map.of(
                                      ...
                                      "properties",
                                          Map.of(
                                              "name",
                                                  Map.of("type", "string", "description", "系列名称"),
                                              "data",
                                                  Map.of(
                                                      "type", "array",
  ...
  public ToolExecutionResult execute(Map<String, Object> args, ToolContext ctx) {
    ...
    return ToolExecutionResult.builder("图表已成功渲染到用户界面。").block(event).build();
  }
}
