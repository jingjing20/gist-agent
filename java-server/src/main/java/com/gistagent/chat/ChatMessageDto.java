...
/**
 * 内部中性的 Chat 消息表示，用于在 DB、Prompt 构建器、Agent Loop 与 OpenAI SDK 之间转换。直接存 OpenAI SDK 的 union
 * 类型既不好持久化也不好估算 token， 所以统一用这个简易 POJO。
 */
public record ChatMessageDto(
  ...
  public static ChatMessageDto assistant(
      String content, List<ToolCallDto> toolCalls, String reasoningContent) {
