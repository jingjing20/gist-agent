...
/**
 * POST /chat 入口：立刻返回 {@link SseEmitter} 把 HTTP 连接交给 Spring， 真正的 Agent 循环在 {@link AsyncTaskExecutor}
 * 里异步跑，通过 emitter 推送事件。
 *
 * <p>不在控制器线程里直接跑 handleChat 的原因：handleChat 内部用 Future.get(120s) 做熔断，会阻塞 controller 返回，导致 SseEmitter
 * 迟迟握手不上，前端看到的是 请求 pending 很久才开始吐数据，用户体验极差。
 */
@RestController
@RequestMapping("/chat")
public class ChatController {
  ...
  public ChatController(
      ChatService chatService, ObjectMapper mapper, AsyncTaskExecutor taskExecutor) {
  ...
  public SseEmitter chat(
      @AuthenticationPrincipal AuthenticatedUser user, @RequestBody ChatRequest body) {
        ...
        () -> {
          try {
            chatService.handleChat(
                stream, userId, body.message(), body.conversationId(), body.datasourceId());
          } catch (Exception e) {
